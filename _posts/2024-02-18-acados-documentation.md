...
`QUADROTOR_ODE_NY0` : We use `yref` to set elements of the cost function, and this is the length of `yref` for first iteration.
`QUADROTOR_ODE_NY` : Length of `yref` for iterations upto `N-1`.
`QUADROTOR_ODE_NYN` : Length of `yref` for the last iteraton, is smaller than `QUADROTOR_ODE_NY` by the size of control vector(`QUADROTOR_ODE_NU`) because control reference is not provided for the last shooting node.
